...
class SolverState(rx.State):
    ...
    def board_squares(self) -> list[dict[str, str | int | bool]]:
        """Pre-computed list of squares for the current solution view."""
        rows = self.result_rows
        cols = self.result_cols
        if rows == 0 or cols == 0:
            return []
        positions: set[tuple[int, int]] = set()
        if self.has_result and self.total_solutions > 0:
            idx = max(
    ...
    @rx.event
    def set_max_solutions(self, v: str):
        self.max_solutions_input = v
